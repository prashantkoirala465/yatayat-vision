"""End-to-end Module 4 demo: detect -> track -> speed -> violation dedup -> DB.
...
frames landing in Postgres out.
    ...
    try:
        for frame_index, (frame, tracks) in enumerate(tracker.track_with_frames(args.source)):
            for det in tracks.detections:
                ...
                session.commit()
                flagged_count += 1
                print(f"flagged: track {flagged.track_id} at {flagged.speed_kmh:.1f} km/h (frame {flagged.frame_index})")
    finally:
        session.close()
